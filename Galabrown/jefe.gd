...
func _esquivar(delta):
	...
	position.x += dir_huida * VEL_ESQUIVE * delta
...
func _atacar_embestida():
	...
	# Bloquea trayectoria y pasa de largo
...
func _ejecutar_embestida(delta):
	...
	if not regresando:
		...
		mov_x_ant = sign(
			objetivo_embestida.x - position.x
		)
		...
		position = position.move_toward(
			pos_origen,
			VEL_EMBESTIDA * 0.6 * delta
		)

		mov_x_ant = sign(
			pos_origen.x - position.x
		)
